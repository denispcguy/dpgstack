...
class TestTableChildView:
    url = reverse('_blueprint:table_child')

    def url_detail(self, pk):
        return reverse('_blueprint:table_child_detail', args=[pk])
    ...
    @pytest.fixture
    def parent(self):
        return f.BlueprintSimpleModelFactory.create()
    ...
    def test_get_normal_row(self, client: Client, parent):
        action = 'normal-row'
        payload = dict(action=action)
    ...
    def test_get_edit_row(self, client: Client, parent):
        action = 'edit-row'
        payload = dict(action=action)
        r: HttpResponse = client.get(self.url_detail(1), data=payload)
        assert r.status_code == 200
    ...
    def test_post_accept_form(self, client: Client, parent):
        action = 'accept-form'
        name = 'New BlueprintSimpleModel'
        payload = dict(action=action, name=name, description='A description', parent_pk=parent.pk)
        r: HttpResponse = client.post(self.url, data=payload)
        assert r.status_code == 200
